...
fn setup_arch_fs(options: &SetupOptions) -> StageOutput {
    ...
    let temp_file = context.data_dir.join("postmarketos-aarch64.tar.xz");
    let fs_root = Path::new(ARCH_FS_ROOT);
    let extracted_dir = context.data_dir.join("chroot_rootfs_qemu-aarch64");
    ...
    if need_setup {
        return Some(thread::spawn(move || {
            ...
            loop {
                if !temp_file.exists() {
                    mpsc_sender
                        .send(SetupMessage::Progress(
                            "Downloading PMOS FS...".to_string(),
                        ))
                        .expect("Failed to send log message");

                    let response = reqwest::blocking::get(ARCH_FS_ARCHIVE)
                        .expect("Failed to download PMOS FS");

                    let total_size = response.content_length().unwrap_or(0);
                    let mut file = File::create(&temp_file)
                        .expect("Failed to create temp file for PMOS FS");
                ...
                let tar_file =
                    File::open(&temp_file).expect("Failed to open downloaded PMOS FS file");
...
fn install_dependencies(options: &SetupOptions) -> StageOutput {
    ...
    return Some(thread::spawn(move || {
        ...
        if let Err(e) = fs::write(&resolv_conf, dns_content) {
            mpsc_sender.send(SetupMessage::Error(format!("Failed to setup DNS: {}", e))).unwrap_or(());
            return;
        }
