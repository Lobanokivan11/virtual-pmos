...
fn default_install() -> String {
    "apk add --no-cache --no-interactive dbus dbus-x11 dbus-systemd systemd-libs flatpak discover-backend-flatpak pcmanfm-qt qps qterminal screengrab xdg-desktop-portal-kde openbox labwc breeze-icons onboard postmarketos-ui-plasma-mobile"
        .to_string()
}

fn default_launch() -> String {
    "mkdir -p /tmp/runtime && chmod 700 /tmp/runtime && XDG_RUNTIME_DIR=/tmp/runtime WAYLAND_DISPLAY=wayland-0 /usr/lib/plasma-dbus-run-session-if-needed startplasmamobile 2>&1"
        .to_string()
}
