...
fn default_install() -> String {
    "apk add --no-cache --no-interactive dbus dbus-x11 systemd-libs flatpak discover-backend-flatpak pcmanfm-qt qps qterminal screengrab xdg-desktop-portal-kde openbox labwc breeze-icons onboard postmarketos-ui-plasma-mobile"
        .to_string()
}
