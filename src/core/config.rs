...
fn default_install() -> String {
    "apk add --no-cache --no-interactive flatpak discover-backend-flatpak noto-fonts pcmanfm-qt qps qterminal screengrab xdg-desktop-portal-kde openbox xorg-xwayland labwc breeze-icons onboard postmarketos-ui-plasma-mobile"
        .to_string()
}
